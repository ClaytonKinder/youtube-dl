...
if (fs.existsSync(detailsPath)) {
  ...
  ytdlBinary = (details.path) ? details.path : (isWin ? path.resolve(__dirname, '..', 'bin', details.exec_win) : path.resolve(__dirname, '..', 'bin', details.exec));
}

// Check that youtube-dl file exists.
if (!fs.existsSync(ytdlBinary)) {
  console.log('ERROR: unable to locate youtube-dl details in ' + path.dirname(ytdlBinary)+'||'+detailsPath+'||'+JSON.stringify(details));
  process.exit(1);
}
